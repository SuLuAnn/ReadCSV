...
namespace DataTraning
{
    public class Futures
    {
        ...
        public string GetTodayData()
        {
            ...
            return Global.HtmlPost(Global.DAY_FUTURES, headers, "BIG5");
        }
        ...
        {
            return SpiltData().Where(data =>data[(int)Global.Futures.TRADING_HOURS] == "盤後")
                                            .Select(data => new 日期貨盤後行情表_luann
                                            {
                                                交易日期 = data[(int)Global.Futures.TRANSACTION_DATE].Replace("/", string.Empty),
                                                契約 = data[(int)Global.Futures.CONTRACT],
                                                到期月份_週別_ = data[(int)Global.Futures.EXPIRY_MONTH],
                                                開盤價 = decimal.TryParse(data[(int)Global.Futures.OPENING_PRICE], out decimal openPrice) ? (decimal?)openPrice : null,
                                                最高價 = decimal.TryParse(data[(int)Global.Futures.HIGHEST_PRICE], out decimal highPrice) ? (decimal?)highPrice : null,
                                                最低價 = decimal.TryParse(data[(int)Global.Futures.LOWEST_PRICE], out decimal lowPrice) ? (decimal?)lowPrice : null,
                                                收盤價 = decimal.TryParse(data[(int)Global.Futures.CLOSING_PRICE], out decimal closePrice) ? (decimal?)closePrice : null
                                            });
        }
        public void AddReviseFutursDetail()
        {
            List<日期貨盤後行情表_luann> futures = GetFutursDetail().ToList();
            Global.SaveCsv<日期貨盤後行情表_luann>(futures, $"{DateTime.Now.Year}_日期貨盤後行情表.csv");
        ...
        public void AddReviseFutursStatistic()
        {
            ...
            Global.SaveCsv<日期貨盤後統計表_luann>(futures, $"{DateTime.Now.Year}_日期貨盤後統計表.csv");
